...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('categories', function (Blueprint $table) {
            ...
            $table->foreignUuid('room_id')->nullable()->after('group_id')->constrained('lookups')->onDelete('set null');
            $table->string('product_type', 20)->nullable();
            $table->string('display_name');
            $table->string('slug', 64);
            $table->text('description')->nullable();
            $table->jsonb('filterable_specs')->default('[]');
        ...
        DB::statement('CREATE INDEX idx_categories_room ON categories (room_id) WHERE room_id IS NOT NULL');

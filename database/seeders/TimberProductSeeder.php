...
class TimberProductSeeder extends Seeder
{
    ...
    {
        $sofaCategoryId = DB::table('categories')->where('slug', 'ghe-sofa')->value('id');
        $doiCategoryId = DB::table('categories')->where('slug', 'ghe-doi')->value('id');
        $collectionId = DB::table('collections')->where('slug', 'timber')->value('id');

        if (! $sofaCategoryId || ! $doiCategoryId) {
            $this->command->error('Main categories "ghe-sofa" or "ghe-doi" not found. Run CategorySeeder first.');
            return;
        }

        $sofaProductId = $this->createProduct($sofaCategoryId, $collectionId, 'Sofa');
        $doiProductId = $this->createProduct($doiCategoryId, $collectionId, "Đôi");

        foreach ($this->variants as $variantData) {
            $targetProductId = str_contains($variantData['slug'], '-sofa-')
                ? $sofaProductId
                : $doiProductId;

            $variantId = $this->createVariant($targetProductId, $variantData);
            $this->attachImages($variantId, $variantData['folder'], $variantData['images']);
        }

        $this->command->info('Timber products seeded into separate categories.');
    }

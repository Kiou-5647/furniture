...
import { index, ship, deliver, cancel, resend, returnItem as returnItemRoute } from '@/routes/employee/fulfillment/shipments';
...
function confirmReturn() {
    ...
    router.post(returnItemRoute({ shipment: props.shipment.id, shipmentItem: returnItem.value.id }).url, {
        ...
        <div v-if="shipment" class="space-y-4 p-4">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    <Badge
                        :class="['text-xs', shipment.status_color ? `text-${shipment.status_color}-600` : '']"
                        variant="outline"
                    >
                        <component
                            :is="shipment.status === 'delivered' ? CheckCircle2 : shipment.status === 'cancelled' ? Ban : shipment.status === 'shipped' ? Truck : Package"
                ...
                <div v-if="shipment.order" class="rounded-lg border p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between">
                            ...
                            <span class="font-mono">{{ shipment.order.order_number }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-muted-foreground">Tổng tiền</span>
                            <span class="font-medium tabular-nums">{{ Number(shipment.order.total_amount).toLocaleString('vi-VN') }}đ</span>
                        </div>
                        <div v-if="parseFloat(shipment.order.shipping_cost) > 0" class="flex justify-between">
                            <span class="text-muted-foreground">Phí ship</span>
                            <span class="tabular-nums">{{ Number(shipment.order.shipping_cost).toLocaleString('vi-VN') }}đ</span>
                        </div>
                        <p v-if="shipment.order.notes" class="text-muted-foreground">
                ...
                <div class="rounded-lg border p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        <template v-if="shipment.order?.customer">
                            <p class="font-medium">{{ shipment.order.customer.name }}</p>
                            <p class="text-muted-foreground">{{ shipment.order.customer.email }}</p>
                        </template>
                        <template v-else>
                            <p class="font-medium">{{ shipment.order?.guest_name || 'Khách vãng lai' }}</p>
                            <p v-if="shipment.order?.guest_phone" class="text-muted-foreground">{{ shipment.order.guest_phone }}</p>
                            <p v-if="shipment.order?.guest_email" class="text-muted-foreground">{{ shipment.order.guest_email }}</p>
                        </template>
                    </div>
                </div>
                ...
                <div class="rounded-lg border p-4">
                    ...
                    <div class="space-y-2 text-sm">
                        ...
                        <template v-if="shipment.handled_by">
                            <p class="font-medium">{{ shipment.handled_by.full_name }}</p>
                            <p class="text-xs text-muted-foreground">{{ shipment.handled_by.phone ?? '—' }}</p>
                        </template>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-lg border">
                <div class="px-4 py-3 border-b">
                    <h3 class="flex items-center gap-2 text-sm font-medium">
                        <Package class="h-4 w-4" /> Sản phẩm ({{ shipment.items?.length ?? 0 }})
                    </h3>
                </div>
                <table class="w-full table-fixed">
                    <thead>
                        <tr class="border-b bg-muted/50 text-xs text-muted-foreground">
                            <th class="w-[30%] px-4 py-2 text-left">Sản phẩm</th>
                            <th class="w-[8%] px-4 py-2 text-center">SL</th>
                            <th class="w-[20%] px-4 py-2 text-center">Nguồn</th>
                            <th class="w-[15%] px-4 py-2 text-center">Trạng thái</th>
                            <th class="w-[15%] px-4 py-2 text-center">Thao tác</th>
                            <th class="w-[12%] px-4 py-2 text-center">Người gửi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr v-for="item in shipment.items" :key="item.id" class="border-b text-sm">
                            <td class="w-[30%] truncate px-4 py-3">{{ item.order_item?.purchasable_name ?? '—' }}</td>
                            <td class="w-[8%] px-4 py-3 text-center tabular-nums">{{ item.quantity_shipped }}</td>

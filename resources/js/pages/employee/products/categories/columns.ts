...
export function getColumns(
    ...
    const columns: ColumnDef<Category>[] = [
        // Cột Ảnh
        {
            ...
            cell: ({ row }) => {
                ...
                if (!item.image_thumb_url) return h('div', { class: 'text-[10px] text-muted-foreground italic' }, 'Không ảnh');
    ...
    if (showGroupColumn) {
        columns.push({
            ...
            header: 'Nhóm',
            size: 150,
            cell: ({ row }) => h(Badge, { variant: 'outline', class: 'bg-primary/5 border-primary/20 text-primary' }, () => row.original.group?.display_name ?? 'N/A')
        });
    }
    ...
    columns.push(
        {
            ...
            size: 120,
            meta: { align: 'center' },
            cell: ({ row }) => h(Badge, { variant: 'secondary' }, () => row.original.product_type_label)
        },
        {
            accessorKey: 'slug',
            header: 'Khóa',
            size: 150,
            meta: { align: 'center' },
            cell: ({ row }) => h('span', { class: 'text-xs text-muted-foreground tabular-nums' }, row.original.slug)
        },
        {
        ...
        // Cột Thời gian
        {
            ...
            size: 160,
            meta: { align: 'center' },
            cell: ({ row }) => h('span', { class: 'text-xs text-muted-foreground tabular-nums' }, row.original.updated_at)
        },
        // Cột Thao tác
        {
            id: 'actions',

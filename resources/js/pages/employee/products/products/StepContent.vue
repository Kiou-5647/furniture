...
<template>
    <div class="space-y-6 pb-10">
        ...
        <div class="flex items-center justify-between">
            <div class="space-y-1">
                <Heading title="Chi tiết sản phẩm" description="Quản lý thông tin, tính năng và thông số kỹ thuật" />
            </div>
        </div>
            ...
            <CardContent class="p-6">
                <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                    ...
                    <div class="md:col-span-2">
                        <Field>
                            <FieldLabel for="name">Tên sản phẩm
                                <span class="text-destructive">*</span>
                            </FieldLabel>
                            <FieldContent>
                                <Input id="name" v-model="ctx.form.name" placeholder="VD: Sofa Sven da thật"
                        ...
                        <Field>
                            ...
                            <FieldContent>
                                <Select v-model="ctx.form.vendor_id">
                                    ...
                                    <SelectContent class="min-w-(--radix-select-trigger-width)">
                                        <SelectItem :value="null">Không có</SelectItem>
                                        <SelectItem v-for="v in vendorOptions" :key="v.id" :value="v.id">
                                            {{ v.label }}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                    ...
                    <div class="md:col-span-1">
                        <Field>
                            ...
                            <FieldContent>
                                ...
                                <FieldError :errors="[ctx.form.errors.category_id]" />
                            </FieldContent>
                        </Field>
                    </div>
                        ...
                        <Field>
                            ...
                            <FieldContent>
                                <Select v-model="ctx.form.collection_id">
                                    ...
                                    <SelectContent class="min-w-(--radix-select-trigger-width)">
                                        <SelectItem v-for="c in collectionOptions" :key="c.id" :value="c.id">
                                            {{ c.label }}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                                <FieldError :errors="[ctx.form.errors.collection_id]" />
                            </FieldContent>
                        </Field>
                    </div>
                        ...
                        <Field>
                            ...
                            <FieldContent>
                                <Select v-model="ctx.form.status">
                                    ...
                                    <SelectContent class="min-w-(--radix-select-trigger-width)">
                                        <SelectItem v-for="s in ctx.statusOptions" :key="s.value" :value="s.value">
                                            {{ s.label }}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                    ...
                    <div class="md:col-span-1">
                        <Field>
                            ...
                            <FieldContent>
                                <Input :model-value="ctx.form.warranty_months ?? undefined
                                    " @update:model-value="
                                        ctx.form.warranty_months = $event
                                            ? Number($event)
                                            : null
                                        " />
                                <FieldError :errors="[ctx.form.errors.warranty_months]" />
                            </FieldContent>
                        </Field>
                    </div>
                            ...
                            <FieldContent>
                                <Popover v-slot="{ close }">
                                    ...
                                    <PopoverContent class="w-auto p-0" align="start">
                                        <Calendar :model-value="ctx.form.published_date
                                                ? parseDate(
                                                    ctx.form
                                                        .published_date,
                                                )
                    ...
                    <div v-if="ctx.form.status === 'published'" class="md:col-span-1">
                        <Field>
                            <FieldLabel>Thời gian hàng mới</FieldLabel>
                            <FieldContent>
                                <div v-if="!ctx.form.new_arrival_until" class="flex gap-2">
                                    <Select :model-value="ctx.newArrivalMonths
                                        ...
                                        <SelectContent>
                                            <SelectItem v-for="m in [1, 2, 3, 6, 12]" :key="m" :value="String(m)">{{ m
                                                }} tháng</SelectItem>
                                        </SelectContent>
                                    </Select>
                                    <Input :model-value="ctx.newArrivalMonths ?? ''
                                        " @update:model-value="
                                            ctx.newArrivalMonths = $event
                                                ? Number($event)
                                                : null
                                            " placeholder="Nhập" class="w-20 text-sm" />
                                </div>
                                <Popover v-else v-slot="{ close }">
                                    ...
                                    <PopoverContent class="w-auto p-0" align="start">
                                        <Calendar :model-value="ctx.form.new_arrival_until
                                                ? parseDate(
                                                    ctx.form
                                                        .new_arrival_until,
                                                )
                                            ...
                                            " :default-placeholder="undefined" layout="month-and-year" initial-focus />
                                    </PopoverContent>
                                </Popover>
                            </FieldContent>
                        </Field>
        ...
        <div v-if="ctx.form.new_arrival_until"
        ...
        <!-- Features Section -->
        <Accordion type="single" collapsible v-model="activeAccordion" class="space-y-4">
            <AccordionItem value="features" class="overflow-hidden rounded-xl border bg-card shadow-sm">
                <AccordionTrigger class="px-6 py-4 transition-colors hover:bg-muted/50 hover:no-underline">
                ...
                <AccordionContent class="space-y-6 p-6">
                    ...
                    <div v-if="ctx.featureOptions.length > 0" class="space-y-3">
                        ...
                        <div
                            ...
                            <label v-for="opt in ctx.featureOptions" :key="opt.id"
                                ...
                                :class="ctx.form.features.some(
                                    (f: SpecItem) =>
                                        ...
                                        : ''
                                    ">
                                <Checkbox :model-value="ctx.form.features.some(
                                    (f: SpecItem) =>
                                        f.display_name === opt.label,
                                )
                                    " @update:model-value="
                    ...
                    <div v-if="ctx.form.features.length > 0" class="space-y-3">
                        ...
                        <div class="grid grid-cols-1 gap-3">
                            <div v-for="(feature, i) in ctx.form.features" :key="i"
                                ...
                                <Textarea v-model="feature.description" placeholder="Mô tả chi tiết đặc điểm nổi bật..."
                    ...
                    <div class="space-y-3 border-t pt-4">
                        ...
                        <div class="grid grid-cols-1 gap-3">
                            <div v-for="(item, idx) in newFeatureItems" :key="idx"
                                class="space-y-3 rounded-xl border bg-muted/20 p-4">
                                <div class="flex items-center gap-2">
                                    <Input v-model="item.display_name" placeholder="Tên tính năng tùy chỉnh"
                                        class="h-9" />
                                    <Button type="button" variant="ghost" size="sm" class="h-9 w-9 p-0 text-destructive"
                        ...
                        <Button v-if="newFeatureItems.length > 0" type="button" variant="secondary" size="sm"
                            class="w-full" @click="addFreeFeatureItems">
            ...
            <AccordionItem value="specs" class="overflow-hidden rounded-xl border bg-card shadow-sm">
                <AccordionTrigger class="px-6 py-4 transition-colors hover:bg-muted/50 hover:no-underline">
                    <div class="text-md flex items-center gap-3 font-semibold">
                        <div class="rounded-md bg-primary/10 p-1.5">
                ...
                <AccordionContent class="space-y-6 p-6">
                    ...
                    <div class="flex items-center justify-between border-b pb-4">
                        <Button type="button" variant="default" size="sm" class="text-xs" @click="openNewGroupForm()">
                    ...
                    " class="space-y-4 rounded-xl border bg-muted/40 p-5 ring-2 ring-primary/20">
                        <div class="flex items-center justify-between">
                            ...
                            <Button type="button" variant="ghost" size="icon" class="h-7 w-7 p-0"
                        ...
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div class="space-y-2">
                                <Label class="text-xs text-muted-foreground uppercase">Tên nhóm</Label>
                                <Input v-model="ctx.specGroupName" placeholder="VD: Kích thước, Chất liệu..." />
                            </div>
                            <div class="space-y-2">
                                <Label class="text-xs text-muted-foreground uppercase">Nhóm tra cứu</Label>
                                <Select v-model="ctx.specGroupNamespace" @update:model-value="setSpecGroupWithLabel!">
                                    <SelectTrigger class="w-full text-sm">
                                        <SelectValue />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="_null">Sử dụng nhập thủ công</SelectItem>
                        ...
                        " class="flex items-center gap-3 rounded-lg border bg-background p-3">
                            <Switch id="spec_filterable" v-model="ctx.specGroupIsFilterable" class="h-4 w-7" />
                            <div class="flex flex-col">
                                <Label for="spec_filterable" class="cursor-pointer text-sm font-medium">Cho phép lọc sản
                                    phẩm</Label>
                        ...
                        " class="mt-4 space-y-3">
                            ...
                            <div v-if="specLookupOptions.length > 0" class="grid grid-cols-1 gap-2">
                                <div v-for="opt in specLookupOptions" :key="opt.id"
                                    class="flex items-center gap-3 rounded-lg border p-2 transition-all" :class="ctx.specSelectedValues.includes(
                                        opt.slug,
                                    )
                                            ? 'border-primary bg-primary/5'
                                            : 'bg-background'
                                        ">
                                    <Checkbox :model-value="ctx.specSelectedValues.includes(
                                        opt.slug,
                                    )
                                        " @update:model-value="
                                    ...
                                    <div class="flex flex-1 items-center gap-2">
                                        ...
                                        " :model-value="getSpecValueDescription(
                                                opt.slug,
                                            )
                                                " @update:model-value="
                                                ...
                                                " placeholder="Ghi chú giá trị này..." class="ml-auto h-7 w-1/2 text-xs" />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3 border-t pt-4">
                            <div class="flex items-center justify-between">
                                <Label class="text-xs font-bold text-muted-foreground uppercase">Chi tiết bổ
                                    sung</Label>
                            ...
                            <div v-for="(item, idx) in ctx.specGroupItems" :key="idx" class="flex gap-2">
                                <Input v-model="item.display_name" placeholder="Tên thông số"
                                    class="h-8 flex-1 text-xs" />
                                <Input v-model="item.description" placeholder="Giá trị" class="h-8 flex-1 text-xs" />
                        ...
                        <div class="flex justify-end gap-3 pt-4">
                            <Button variant="ghost" size="sm" @click="cancelEditGroup()">Hủy bỏ</Button>
                            <Button size="sm" :disabled="!ctx.specGroupName.trim()" @click="
                                ctx.addSpecGroup();
                            closeSpecForm();
                            ">
                        ...
                        <div v-for="(group, name) in ctx.form.specifications" :key="name"
                            ...
                            <div @click="toggleGroupExpand(String(name))"
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <Badge v-if="group.is_filterable" variant="secondary"
                                        class="px-1.5 py-0 text-[9px]">Lọc</Badge>
                                </div>
                                <ChevronRight class="h-4 w-4 text-muted-foreground transition-transform" :class="{
                            ...
                            <div v-if="expandedGroups.has(String(name))"
                                class="space-y-3 border-t bg-muted/10 p-4 pt-0">
                                <div v-for="item in group.items" :key="item.display_name"
                                    class="flex items-center border-b border-muted py-1 text-xs last:border-0">
                                    <span class="min-w-25 text-muted-foreground">
                                        {{ item.display_name }}
                                    </span>
            ...
            <AccordionItem value="care" class="overflow-hidden rounded-xl border bg-card shadow-sm">
                <AccordionTrigger class="px-6 py-4 transition-colors hover:bg-muted/50 hover:no-underline">
                    <div class="text-md flex items-center gap-3 font-semibold">
                        <div class="rounded-md bg-primary/10 p-1.5">
                ...
                <AccordionContent class="p-6">
                    <div class="space-y-6">
                        <!-- Instructions Grid -->
                        <div v-if="ctx.form.care_instructions.length > 0"
                            class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            <div v-for="(item, i) in ctx.form.care_instructions" :key="i"
                                ...
                                <span class="text-sm leading-relaxed text-muted-foreground group-hover:text-foreground">
                                    {{ item }}
                                </span>
                        ...
                        <div
                            ...
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                                <div class="flex-1">
                                    <Input v-model="ctx.newCareInstruction" placeholder="Nhập hướng dẫn bảo quản mới..."
            ...
            <!-- Assembly Section -->
            <AccordionItem value="assembly" class="overflow-hidden rounded-xl border bg-card shadow-sm">
                <AccordionTrigger class="px-6 py-4 transition-colors hover:bg-muted/50 hover:no-underline">
                    <div class="text-md flex items-center gap-3 font-semibold">
                        <div class="rounded-md bg-primary/10 p-1.5">
                            <Settings class="h-4 w-4 text-primary" />
                ...
                <AccordionContent class="p-6">
                    <div class="space-y-6">
                        <!-- Master Switch -->
                        <div class="flex items-center justify-between rounded-xl p-4 transition-all" :class="ctx.form.assembly_info.required
                                ? 'bg-primary/5 ring-1 ring-primary/20'
                                : 'bg-muted/30'
                            ">
                            <div class="flex items-center gap-3">
                                <div class="flex h-8 w-8 items-center justify-center rounded-lg" :class="ctx.form.assembly_info.required
                            ...
                            <Switch id="assembly_required" v-model="ctx.form.assembly_info.required" class="h-5 w-9" />
                        </div>
                        ...
                        <template v-if="ctx.form.assembly_info.required">
                            <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
                                ...
                                <div class="space-y-4 rounded-2xl border bg-background p-5 shadow-sm">
                                    <div class="flex items-center gap-2 border-b pb-3">
                                        <span
                                            class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Thông
                                            số kỹ thuật</span>
                                    </div>
                                    <div class="grid grid-cols-2 gap-4">
                                        <Field class="space-y-2">
                                            <FieldLabel class="text-[11px] font-semibold">Thời gian (phút)</FieldLabel>
                                            <FieldContent>
                                                <Input :model-value="ctx.form.assembly_info
                                                        .estimated_minutes ??
                                                    ''
                                                    " @update:model-value="
                                                        ...
                                                        $event
                                                            ? Number($event)
                                                            : null
                                                        " type="number" placeholder="30" class="h-9" />
                                            </FieldContent>
                                        </Field>
                                        <Field class="space-y-2">
                                            <FieldLabel class="text-[11px] font-semibold">Mức độ khó</FieldLabel>
                                            <FieldContent>
                                                <Select v-model="ctx.form.assembly_info
                                                        .difficulty_level
                                                    ">
                                                    ...
                                                    <SelectContent>
                                                        <SelectItem v-for="d in ctx.difficultyOptions" :key="d.value"
                                                            :value="d.value">
                                                            {{ d.label }}
                                                        </SelectItem>
                                                    </SelectContent>
                                ...
                                <div class="space-y-4 rounded-2xl border bg-background p-5 shadow-sm">
                                    <div class="flex items-center gap-2 border-b pb-3">
                                        <span
                                            class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Tài
                                            liệu hướng dẫn</span>
                                    </div>
                                    <div class="space-y-3">
                                        <div
                                            ...
                                            <div class="flex flex-col items-center text-center">
                                                <div class="mb-2 rounded-full bg-muted p-2 text-muted-foreground">
                                                    <Plus class="h-4 w-4" />
                                                </div>
                            ...
                            <!-- Full Width Note -->
                            <Field class="space-y-2">
                                <FieldLabel class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Ghi
                                    chú bổ sung</FieldLabel>
                                <FieldContent>
                                    <Textarea :model-value="ctx.form.assembly_info

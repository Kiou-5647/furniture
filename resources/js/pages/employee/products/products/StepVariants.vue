...
<template>
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <div class="flex flex-col gap-1">
                <div class="flex items-center gap-2">
                    ...
                    <Label class="text-xl font-bold tracking-tight">Quản lý Biến thể</Label>
                </div>
            ...
            <Button type="button" variant="default"
                ...
                :disabled="ctx.form.option_groups.length === 0" @click="openAutoCreateDialog">
                <div
        ...
        <div v-if="ctx.form.option_groups.length > 0"
            ...
            <div class="flex items-center justify-between border-b bg-muted/30 px-4 py-3">
                <div class="flex items-center gap-2">
                    ...
                    <Label class="text-sm font-semibold">Thêm biến thể thủ công</Label>
                </div>
                <Button type="button" size="sm"
                    class="h-8 rounded-lg text-xs font-bold transition-all hover:bg-primary hover:text-primary-foreground"
                    :disabled="Object.keys(ctx.newVariantCombo).length !==
            ...
            <div class="p-4">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div v-for="group in ctx.form.option_groups" :key="group.name" class="space-y-1.5">
                        <Label class="text-[11px] font-bold tracking-wider text-muted-foreground uppercase">
                            {{ group.name }}
                        </Label>
                        <Select :model-value="ctx.newVariantCombo[group.namespace] ?? ''
                            " @update:model-value="
                                ctx.newVariantCombo = {
                                    ...ctx.newVariantCombo,
                                ...
                                ">
                            <SelectTrigger class="h-9 rounded-lg text-sm focus:ring-primary">
        ...
        <div v-if="ctx.form.variants.length > 0" class="space-y-4">
            <div class="flex items-center justify-between px-1">
                <div class="flex items-center gap-2">
                    <Label class="text-sm font-bold tracking-widest text-muted-foreground uppercase">
                        Danh sách biến thể
                    </Label>
                    <Badge variant="secondary" class="rounded-full px-2 py-0 text-[10px] font-bold">
                        {{ ctx.form.variants.length }}
                    </Badge>
                </div>
            </div>
            <Accordion type="single" collapsible v-model="activeVariantAccordion" class="flex flex-col gap-3">
                <AccordionItem v-for="(variant, vi) in ctx.form.variants" :key="vi" :value="String(vi)"
                    ...
                    <AccordionTrigger class="px-4 py-3 hover:no-underline">
                        <div class="flex w-full items-center gap-4">
                            ...
                            <div class="flex flex-1 flex-wrap gap-1.5">
                                <Badge v-for="(
label, li
                                    ) in ctx.getVariantOptionLabels(variant)" :key="li" variant="outline"
                            ...
                            <div class="flex shrink-0 items-center gap-4">
                                <div class="flex flex-col items-end gap-0.5 md:flex">
                                ...
                                <div class="flex items-center gap-1">
                                    <Button type="button" variant="ghost" size="sm"
                                        class="h-8 w-8 p-0 transition-colors" :class="justCopied
                                                ? 'text-green-500'
                                                : 'text-muted-foreground hover:text-foreground'
                                            " @click.stop="copyVariantData(variant)">
                                        <ClipboardCheck v-if="justCopied" class="h-4 w-4" />
                                        <Clipboard v-else class="h-4 w-4" />
                                    </Button>
                                    <Button type="button" variant="ghost" size="sm"
                                        class="h-8 w-8 p-0 transition-colors" :class="copiedVariantData
                                                ? 'text-primary'
                                                : 'text-muted-foreground hover:text-foreground'
                                            " :disabled="!copiedVariantData" @click.stop="
                    ...
                    <AccordionContent>
                        <div
                            ...
                            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                                <Field class="md:col-span-2">
                                    <FieldLabel class="text-sm font-bold">Tiêu đề biến thể</FieldLabel>
                                    <div class="flex items-center gap-2">
                                        <span v-if="ctx.form.name"
                                            class="text-sm font-medium whitespace-nowrap text-muted-foreground">
                                            {{ ctx.form.name }}
                                        </span>
                                        <Input :model-value="variant.name" @update:model-value="
                                            variant.name = $event
                                            " placeholder='VD: 48" Black Table' class="h-9 flex-1 text-sm" />
                                    </div>
                                </Field>

                                <Field>
                                    <FieldLabel
                                        class="text-xs font-bold tracking-wider text-muted-foreground uppercase">URL
                                        Slug</FieldLabel>
                                    <div class="relative">
                                        <Input :model-value="getVariantSlug(variant)
                                            " readonly class="h-9 border-dashed bg-background font-mono text-xs" />
                                        <div
                                ...
                                <Field>
                                    <FieldLabel
                                        class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Trạng
                                        thái</FieldLabel>
                                    <Select v-model="variant.status">
                                        ...
                                        <SelectContent>
                                            <SelectItem value="active" class="text-sm">Hoạt động</SelectItem>
                                            <SelectItem value="inactive" class="text-sm">Không hoạt động</SelectItem>
                                        </SelectContent>
                                    </Select>
                                </Field>
                            </div>
                            ...
                            <Field>
                                <FieldLabel class="text-sm font-bold">Mô tả biến thể</FieldLabel>
                                <Textarea :model-value="variant.description" @update:model-value="
                            ...
                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
                                <Field>
                                    <FieldLabel class="text-sm font-bold">SKU</FieldLabel>
                                    <Input v-model="variant.sku" placeholder="VD: A3X9K2M1"
                                        class="h-9 font-mono text-sm" />
                                </Field>
                                <Field>
                                    <FieldLabel class="text-sm font-bold">Nhãn Swatch</FieldLabel>
                                    <Input v-model="variant.swatch_label" placeholder="VD: Midnight Blue"
                                        class="h-9 text-sm" />
                                </Field>
                                <Field>
                                    ...
                                    <Input :model-value="formatNumber(variant.price)
                                        " @input="
                                            (e: any) =>
                                                handleNumericInput(
                                                    e,
                                        ...
                                        " placeholder="0" class="h-9 text-sm font-bold text-primary" />
                                </Field>
                            </div>

                            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                                <Field>
                                    <FieldLabel class="text-sm font-medium">Lợi nhuận (Giá trị)</FieldLabel>
                                    <Input :model-value="variant.profit_margin_value
                                            ? formatNumber(
                                                variant.profit_margin_value,
                                            )
                                            : ''
                                        ...
                                        " placeholder="0" class="h-9 text-sm" />
                                </Field>
                                <Field>
                                    <FieldLabel class="text-sm font-medium">Lợi nhuận (Đơn vị)</FieldLabel>
                        ...
                        <div class="space-y-6 rounded-2xl border bg-muted/20 p-4">
                            <div class="mb-4 flex items-center gap-2">
                                <Camera class="h-4 w-4 text-primary" />
                                <Label class="text-sm font-bold tracking-wider uppercase">Trung tâm hình ảnh</Label>
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                                <!-- Primary Image -->
                                <div class="space-y-2">
                                    <Label class="text-[11px] font-bold text-muted-foreground uppercase">Ảnh
                                        chính</Label>
                                    <div
                                        ...
                                        " :src="variant.primary_image_file
                                                    ? createObjectURL(
                                                        variant.primary_image_file,
                                                    )
                                                    : variant.primary_image_url
                                ...
                                <!-- Dimension Image -->
                                <div class="space-y-2">
                                    <Label class="text-[11px] font-bold text-muted-foreground uppercase">Kích
                                        thước</Label>
                                    <div
                                        class="group relative aspect-square overflow-hidden rounded-xl border bg-background shadow-sm transition-all hover:border-primary/40">
                                        <img v-if="
                                        ...
                                        " :src="variant.dimension_image_file
                                                    ? createObjectURL(
                                                        variant.dimension_image_file,
                                                    )
                                                    : variant.dimension_image_url
                                ...
                                <!-- Swatch Image -->
                                <div class="space-y-2">
                                    <Label class="text-[11px] font-bold text-muted-foreground uppercase">Swatch</Label>
                                    <div
                                        class="group relative aspect-square overflow-hidden rounded-xl border bg-background shadow-sm transition-all hover:border-primary/40">
                                        <img v-if="
                                            variant.swatch_image_url ||
                                            variant.swatch_image_file
                                        " :src="variant.swatch_image_file
                                                    ? createObjectURL(
                                                        variant.swatch_image_file,
                                                    )
                                                    : variant.swatch_image_url
                            ...
                            <div class="space-y-3 border-t pt-4">
                                ...
                                <div class="grid grid-cols-3 gap-3 sm:grid-cols-5 md:grid-cols-8">
                                    <!-- Existing Gallery Images -->
                                    <div v-for="(
img, idx
                                        ) in variant.gallery_urls" :key="img.id"
                                    ...
                                    <div v-for="(
file, idx
                                        ) in variant.gallery_files" :key="idx"
                                        class="group relative aspect-square overflow-hidden rounded-lg border bg-background shadow-sm">
                                    ...
                                    <!-- Upload Trigger -->
                                    <label v-if="canAddMoreImages(variant)"
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center gap-2">
                                ...
                                <Label class="text-sm font-bold tracking-wider uppercase">
                                    Đặc điểm nổi bật
                                </Label>
                            </div>
                            ...
                            <div v-if="ctx.featureOptions.length > 0" class="space-y-2">
                                <Label class="text-[11px] font-bold tracking-tight text-muted-foreground uppercase">
                                    Chọn nhanh từ tra cứu
                                </Label>
                                <div class="flex flex-wrap gap-2">
                                    <button v-for="opt in ctx.featureOptions" :key="opt.slug" type="button" @click="
                            ...
                            <div v-if="(variant.features || []).length > 0" class="space-y-3 border-t pt-4">
                                <Label class="text-[11px] font-bold tracking-tight text-muted-foreground uppercase">
                                    Danh sách tính năng
                                </Label>
                                <div class="grid gap-3">
                                    <div v-for="(
feature, i
                                        ) in variant.features || []" :key="i"
                                        class="group relative space-y-2 rounded-xl border bg-background p-3 transition-colors hover:border-primary/30">
                                        <div class="flex items-center gap-2">
                                            <Input v-model="feature.display_name" placeholder="Tên tính năng"
                                                class="h-8 text-xs font-bold" />
                                            <Button type="button" variant="ghost" size="sm"
                                                class="h-8 w-8 p-0 text-muted-foreground hover:text-destructive" @click="
                                                    (
                                                        variant.features || []
                                                    ).splice(i, 1)
                                                    ">
                                        ...
                                        <Textarea v-model="feature.description" placeholder="Mô tả chi tiết..."
                            ...
                            <div class="space-y-3 rounded-2xl border bg-muted/30 p-4">
                                <div class="flex items-center justify-between">
                                    <Label class="text-[11px] font-bold tracking-tight text-muted-foreground uppercase">
                                        Nhập tự do
                                    </Label>
                                    <Button type="button" variant="ghost" size="sm"
                                        class="h-7 text-[11px] font-bold text-primary hover:bg-primary/10" @click="
                                ...
                                " class="space-y-3">
                                    <div v-for="(
item, idx
                                        ) in getVariantFeatureItems(Number(vi))" :key="idx"
                                        class="space-y-2 rounded-lg border bg-background p-3">
                                        <div class="flex items-center gap-2">
                                            <Input v-model="item.display_name" placeholder="Tên tính năng"
                                                class="h-8 text-xs" />
                                            <Button type="button" variant="ghost" size="sm"
                                                class="h-8 w-8 p-0 text-muted-foreground hover:text-destructive" @click="
                                        ...
                                        <Textarea v-model="item.description" placeholder="Mô tả chi tiết..."
                                            class="h-14 resize-none text-xs" />
                                    </div>
                                    <Button type="button" size="sm" class="h-9 w-full rounded-lg text-xs font-bold"
                                        @click="
                                ...
                                <div v-else class="flex items-center justify-center py-4 text-center">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center justify-between px-1">
                                <div class="flex items-center gap-2">
                                    ...
                                    <Label class="text-sm font-bold tracking-wider uppercase">Thông số kỹ thuật</Label>
                                </div>
                                <Button type="button" variant="ghost" size="sm"
                            ...
                            <div v-else class="space-y-3">
                                <div v-for="(
group, groupName
                                    ) in variant.specifications" :key="groupName"
                                    class="group/spec overflow-hidden rounded-xl border bg-card transition-all hover:border-primary/30">
                                    <div class="flex cursor-pointer items-center justify-between bg-muted/50 px-4 py-2 transition-colors hover:bg-muted/80"
                                        @click="
                        ...
                        <div class="mt-3 space-y-3 rounded-2xl border bg-muted/20 p-4">
                            <div class="mb-2 flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <Settings class="h-4 w-4 text-primary" />
                                    <Label class="text-sm font-bold tracking-wider uppercase">Hướng dẫn bảo quản</Label>
                                </div>
                                <Button type="button" variant="ghost" size="sm"
                                    class="h-7 text-xs font-bold text-primary hover:bg-primary/10"
                            ...
                            <div v-if="variant.care_instructions?.length" class="space-y-2">
                                <div v-for="(
instruction, idx
                                    ) in variant.care_instructions" :key="idx"
                                    ...
                                    <Button variant="ghost" size="sm"
                                        class="h-6 w-6 p-0 text-muted-foreground hover:text-destructive" @click="
            ...
            <!-- FLOATING SPEC FORM -->
            <div v-if="variantSpecEditing !== null"
                ...
                <div
                    ...
                    <div class="flex items-center justify-between border-b bg-muted/30 px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <Label class="text-lg font-bold">Cấu hình Thông số</Label>
                        </div>
                        <Button variant="ghost" size="icon" class="rounded-full" @click="closeVariantSpecForm">
                            <X class="h-5 w-5" />
                        </Button>
                    </div>
                    ...
                    <div class="space-y-6 p-6">
                        ...
                        <div class="mb-6 rounded-xl border border-dashed bg-muted/40 p-3">
                            <div class="flex items-center gap-3">
                                <div class="flex-1 space-y-1">
                                    <Label class="text-[10px] font-bold text-muted-foreground uppercase">Sao chép từ
                                        nhóm sản phẩm</Label>
                                    <Select v-model="selectedProductSpecGroup" @update:model-value="
                                        onSelectProductSpecGroup(
                                            String($event),
                                        )
                                        ">
                                        <SelectTrigger class="h-8 text-xs">
                                            <SelectValue placeholder="Chọn nhóm sản phẩm để sao chép..." />
                                        </SelectTrigger>
                                        <SelectContent>
                                            <SelectItem v-for="name in productSpecGroupNames" :key="name" :value="name"
                                                class="text-xs">
                                                {{ name }}
                                            </SelectItem>
                                        </SelectContent>
                        ...
                        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                            <Field>
                                <FieldLabel class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Tên
                                    nhóm thông số</FieldLabel>
                                <Input v-model="variantSpecGroupName" placeholder="VD: Kích thước chi tiết"
                                    class="h-10" />
                            </Field>
                            <Field>
                                <FieldLabel class="text-xs font-bold tracking-wider text-muted-foreground uppercase">
                                    Nhóm tra cứu (Lookup)</FieldLabel>
                                <Select v-model="variantSpecGroupNamespace">
                                    <SelectTrigger class="h-10">
                                        <SelectValue placeholder="Chọn nhóm tra cứu..." />
                                    </SelectTrigger>
                                    <SelectContent>
                                        <SelectItem value="_null">Tùy chỉnh (Custom)</SelectItem>
                                        <SelectItem v-for="ns in specNamespaces" :key="ns.namespace"
                                            :value="ns.namespace">
                                            {{ ns.label }}
                                        </SelectItem>
                                    </SelectContent>
                        ...
                        <div v-if="variantSpecGroupNamespace !== '_null'" class="flex items-center gap-2">
                            <Switch v-model="variantSpecGroupIsFilterable" :id="`filter-${variantSpecEditingName}`" />
                            <Label for="filter-{{ variantSpecEditingName }}" class="text-sm">
                        ...
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                <Label class="text-sm font-bold tracking-wider text-muted-foreground uppercase">Giá trị
                                    chi tiết</Label>
                                <Button variant="outline" size="sm" class="h-8 text-xs" @click="addSpecFreeItem">
                            ...
                            <div class="grid grid-cols-1 gap-2">
                                <!-- Lookup Options -->
                                <label v-for="opt in variantSpecLookupOptions" :key="opt.slug"
                                    class="group flex items-center gap-3 rounded-xl border p-3 transition-all hover:border-primary/40 hover:bg-primary/5"
                                    :class="variantSpecSelectedValues.includes(
                                        opt.slug,
                                    )
                                            ? 'border-primary bg-primary/5 ring-1 ring-primary/20'
                                            : 'bg-card'
                                        ">
                                    <Checkbox :model-value="variantSpecSelectedValues.includes(
                                        opt.slug,
                                    )
                                        " @update:model-value="
                                    ...
                                    <div class="flex flex-1 items-center gap-2">
                                        <span class="text-sm font-medium whitespace-nowrap">{{ opt.label }}</span>
                                        <Input v-if="
                                            variantSpecSelectedValues.includes(
                                                opt.slug,
                                            )
                                        " v-model="variantSpecValueDescriptions[
                                                opt.slug
                                                ]
                                                " placeholder="Mô tả chi tiết..." class="h-8 flex-1 px-2 py-1 text-xs" />
                                    </div>
                                </label>

                                <!-- Free Items -->
                                <div v-for="(item, idx) in variantSpecFreeItems" :key="idx"
                                    class="space-y-2 rounded-xl border bg-background p-3 transition-colors hover:border-primary/30">
                                    <div class="flex items-center gap-2">
                                        <Input v-model="item.display_name" placeholder="Tên giá trị..."
                                    ...
                                    <Textarea v-model="item.description" placeholder="Mô tả chi tiết cho giá trị này..."
                    ...
                    <div class="flex items-center justify-end gap-3 border-t bg-muted/10 px-6 py-4">
                        <Button variant="ghost" class="rounded-xl" @click="closeVariantSpecForm">Hủy bỏ</Button>
                        <Button class="rounded-xl px-6 font-bold" @click="saveVariantSpecGroup(variantSpecEditing!)">Lưu
                            thay đổi</Button>
                    </div>
                </div>
            </div>
            ...
            <!-- FLOATING CARE FORM -->
            <div v-if="variantCareEditing !== null"
                class="fixed inset-0 z-50 flex items-center justify-center bg-background/80 p-4 backdrop-blur-sm"
                ...
                <div
                    ...
                    <div class="flex items-center justify-between border-b bg-muted/30 px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <Label class="text-lg font-bold">Hướng dẫn bảo quản</Label>
                        </div>
                    ...
                    <div class="space-y-4 p-6">
                        <div class="flex gap-2">
                            <Input v-model="variantCareInstruction" placeholder="Nhập hướng dẫn bảo quản..."
                                class="h-10 flex-1" @keyup.enter="
                                    addVariantCareInstruction(
                                        variantCareEditing!,
                                    )
                                    " />
                            <Button type="button" class="rounded-xl px-4 font-bold" @click="
                                addVariantCareInstruction(
                                    variantCareEditing!,
                                )
                                ">
                        ...
                        <div class="max-h-60 space-y-2 overflow-y-auto pr-2">
                            <div v-for="(instr, idx) in ctx.form.variants[
                                variantCareEditing!
                            ]?.care_instructions" :key="idx"
                    ...
                    <div class="flex items-center justify-end gap-3 border-t bg-muted/10 px-6 py-4">
                        <Button variant="ghost" class="rounded-xl" @click="closeVariantCareForm">Đóng</Button>
                    </div>
                </div>
            </div>
        </div>

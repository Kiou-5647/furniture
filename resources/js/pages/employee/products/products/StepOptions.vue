    ...
    <div class="space-y-10 pb-6">
        ...
        <div class="flex flex-col gap-2">
            <div class="flex items-center gap-2">
                ...
                <Label class="text-lg font-bold tracking-tight">Tùy chọn biến thể</Label>
            </div>
        ...
        <div class="space-y-4">
            ...
            <div class="flex flex-col gap-6 lg:flex-row">
                ...
                <div class="w-full shrink-0 space-y-4 lg:w-80">
                    <div class="space-y-6 rounded-2xl border bg-background p-5 shadow-sm">
                        <div class="space-y-3">
                            <Label class="text-xs font-bold tracking-wider text-muted-foreground uppercase">Nhóm tra
                                cứu</Label>
                            <SearchableSelect v-model="ctx.selectedOptionNamespace" :options="variantOptions"
                                value-key="namespace" label-key="label" placeholder="Tìm nhóm thuộc tính..."
                                class="w-full" />
                        </div>

                        <div v-if="ctx.selectedOptionNamespace" class="space-y-4 border-t pt-4">
                            <div class="flex items-center justify-between">
                                <span class="text-xs font-medium text-muted-foreground">Đã chọn:</span>
                                <Badge variant="secondary" class="px-2 text-[10px]">{{ totalSelectedValues }} mục
                                </Badge>
                            </div>

                            <!-- Primary Action: Add Group -->
                            <Button @click="ctx.addOptionGroup(variantOptions)" :disabled="totalSelectedValues === 0"
                ...
                <!-- RIGHT: VALUE PICKER -->
                <div class="flex-1">
                    <div v-if="ctx.selectedOptionNamespace"
                        class="min-h-[300px] rounded-2xl border bg-background p-6 shadow-sm">
                        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                            <label v-for="opt in ctx.selectedVariantOptions" :key="opt.id"
                                ...
                                :class="ctx.selectedOptionValues.includes(opt.slug)
                                    ? 'border-primary bg-primary/5 ring-1 ring-primary/20'
                                    : 'bg-card'
                                    ">
                                <Checkbox :model-value="ctx.selectedOptionValues.includes(
                                    opt.slug,
                                )
                                    " @update:model-value="
                                ...
                                <div class="flex flex-col overflow-hidden">
                                    ...
                                    <span v-if="opt.description" class="truncate text-[10px] text-muted-foreground">{{
                        ...
                        <div v-if="ctx.selectedVariantOptions.length === 0" class="py-12 text-center">
                            <p class="text-sm text-muted-foreground">
                                Không tìm thấy giá trị nào trong nhóm này.
                            </p>
                        </div>
                    </div>
                    <div v-else class="rounded-2xl border border-dashed bg-muted/20 p-12 text-center">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div v-for="(group, gi) in ctx.form.option_groups" :key="gi"
                    ...
                    <div class="space-y-4 p-4">
                        <div class="flex items-center justify-between">
                            <Label class="text-xs font-semibold text-muted-foreground">Giá trị thuộc nhóm này</Label>
                            <div class="flex items-center gap-2">
                                <Label class="text-[10px] font-medium">Hiển thị màu/ảnh</Label>
                                <Switch :model-value="group.is_swatches" @update:model-value="
                                    toggleSwatch(Number(gi))
                                    " class="h-4 w-7" />
                            </div>
                        </div>

                        <div v-for="(option, oi) in group.options" :key="oi"
                            ...
                            <div v-if="group.is_swatches"
                                ...
                                <template v-if="getFullOption(option.value)">
                                    ...
                                    <img v-if="
                                        ...
                                        getFullOption(option.value)
                                            ?.image_url
                                    " :src="getFullOption(option.value)
                                        ?.image_thumb_url ||
                                        getFullOption(option.value)
                                            ?.image_url!
                                    ...
                                    <Layers v-else class="h-3.5 w-3.5 text-muted-foreground/50" />
                                </template>
                                <Layers v-else class="h-3.5 w-3.5 text-muted-foreground/50" />
                            </div>
                            ...
                            <Badge v-if="
                                ...
                                (option.metadata?.color_hex ||
                                    getFullOption(option.value)?.metadata
                                        ?.color_hex)
                            " variant="outline" class="ml-1 h-4 px-1 font-mono text-[9px] text-muted-foreground">
                                {{
                                    option.metadata?.color_hex ||
                                    getFullOption(option.value)?.metadata
                                        ?.color_hex

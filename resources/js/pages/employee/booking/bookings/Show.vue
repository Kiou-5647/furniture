...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
            ...
            <div
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <Heading :title="`Đặt lịch #${booking.booking_number}`" />
                            <Badge :class="[
                                'text-xs',
                                `border-${booking.status_color}-200 bg-${booking.status_color}-100 text-${booking.status_color}-700`
                            ]" variant="outline">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    <div class="flex items-center gap-2 border-l pl-2">
                        <Button v-if="booking.can_confirm" variant="outline"
                            class="border-blue-200 bg-blue-50/50 text-blue-600" :disabled="isProcessing"
                            @click="handleConfirm">
                            <CheckCircle2 class="mr-2 h-4 w-4" /> Xác nhận
                        </Button>
                        <Button v-if="booking.can_cancel" variant="ghost"
                            class="text-destructive hover:bg-destructive/10" :disabled="isProcessing"
                            @click="handleCancel">
                    ...
                    <div class="overflow-hidden rounded-xl border bg-card shadow-sm">
                        ...
                        <div class="space-y-3 p-4">
                            <div class="space-y-1">
                                ...
                                <p v-if="booking.customer?.phone" class="text-sm text-muted-foreground">
                    ...
                    <div class="overflow-hidden rounded-xl border bg-card shadow-sm">
                        ...
                        <div class="space-y-4 p-4">
                            ...
                            <div class="flex items-center justify-between border-t pt-3">
                                <span class="text-xs text-muted-foreground">Tổng thời lượng</span>
                        ...
                        <div class="space-y-8 p-6">
                            ...
                            <div
                                ...
                                <div class="text-right">
                                    <Badge variant="secondary" class="px-3 py-1 text-xs">Tư vấn theo giờ</Badge>
                                </div>
                            </div>
                            <!-- Deposit Section -->
                            <div class="space-y-3">
                                <div class="flex items-center justify-between">
                                    ...
                                    <Badge
                                        :class="`border-${booking.deposit_invoice?.status_color}-200 bg-${booking.deposit_invoice?.status_color}-100 text-${booking.deposit_invoice?.status_color}-700`"
                                ...
                                <div v-if="booking.deposit_invoice" class="space-y-4 rounded-lg border bg-muted/20 p-4">
                                    <div class="flex justify-between text-sm">
                                        <span class="text-muted-foreground">Số tiền cọc</span>
                                    ...
                                    " class="flex gap-2 pt-2">
                                        <Button size="sm" variant="outline" class="h-8 flex-1 text-xs"
                            ...
                            <div v-if="booking.final_invoice" class="space-y-3">
                                <div class="flex items-center justify-between">
                                    <h4 class="text-sm font-bold">
                                        2. Thanh toán cuối
                                    </h4>
                                    <Badge
                                        :class="`border-${booking.final_invoice?.status_color}-200 bg-${booking.final_invoice?.status_color}-100 text-${booking.final_invoice?.status_color}-700`"
                                        variant="outline" class="text-[10px]">
                                        {{
                                ...
                                <div v-if="booking.final_invoice" class="space-y-4 rounded-lg border bg-muted/20 p-4">
                                    <div class="flex justify-between text-sm">
                                        <span class="text-muted-foreground">Số tiền còn lại</span>
                                        <span class="font-bold tabular-nums">{{
                                            formatPrice(
                                    ...
                                    <div class="flex gap-2 pt-2">
                                        ...
                                        ">
                                            <Button size="sm" variant="outline" class="h-8 w-full text-xs"
                                                @click="handleOpenInvoice">Mở hóa đơn</Button>
                                        </template>
                                        <template v-else-if="
        ...
        <div v-else class="space-y-4 p-12 text-center">
            <Loader2 class="mx-auto h-10 w-10 animate-spin text-muted-foreground" />
        ...
        <VnPayPaymentDialog v-if="showVnPayDialog" :open="showVnPayDialog" :payment-url="vnPayUrl"
            :amount="booking.deposit_invoice?.amount_due ?? '0'" @close="showVnPayDialog = false" />
    </AppLayout>
</template>

...
const canBank = computed(() => props.order?.can_bank);
...
function handleVnPayPayment() {
    ...
    window.location.href = initiate(openInvoice.id).url;
}
...
<template>
        ...
        <div v-if="order" class="space-y-6 p-4 lg:p-6">
            ...
            <div
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <Badge :class="[
                                ...
                                <component :is="order.status === 'completed'
                                        ? CheckCircle2
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <div class="mr-2 flex items-center gap-2">
                        <Badge v-if="order.paid_at" variant="secondary"
                            class="border-green-200 bg-green-100 text-green-700 hover:bg-green-100">
                            Đã thanh toán
                        </Badge>
                        <Badge v-if="order.payment_method" :variant="order.payment_method === 'cod'
                                ? 'secondary'
                                : 'outline'
                            ">
                            <component :is="order.payment_method === 'cod'
                                    ? Truck
                                    : DollarSign
                    ...
                    <div class="flex items-center gap-2 border-l pl-2">
                        <Button v-if="canAccept" variant="outline" class="border-blue-200 bg-blue-50/50 text-blue-600"
                            @click="handleAccept">
                            <CheckCircle2 class="mr-2 h-4 w-4" /> Duyệt đơn
                        </Button>
                        <Button v-if="canMarkPaid" variant="outline"
                            class="border-green-200 bg-green-50/50 text-green-600" @click="handleMarkPaid">
                            <CheckCircle2 class="mr-2 h-4 w-4" /> Thu tiền mặt
                        </Button>
                        <Button v-if="
                            canBank
                        " variant="outline" class="border-purple-200 bg-purple-50/50 text-purple-600"
                            @click="handleVnPayPayment">
                            <CreditCard class="mr-2 h-4 w-4" /> Chuyển khoản
                        </Button>
                        <Button v-if="canComplete" variant="default" class="bg-green-600 hover:bg-green-700"
            ...
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
                ...
                <div class="space-y-6 lg:col-span-4">
                        ...
                        <div class="space-y-3 p-4">
                            <template v-if="order.customer">
                                <div class="space-y-1">
                                    ...
                                    <p v-if="order.guest_phone" class="text-sm text-muted-foreground">
                                        {{ order.guest_phone }}
                                    </p>
                                    <p v-if="order.guest_email" class="text-sm text-muted-foreground">
                                        {{ order.guest_email }}
                                    </p>
                                </div>
                            </template>
                    ...
                    <div class="overflow-hidden rounded-xl border bg-card shadow-sm">
                        ...
                        <div class="space-y-3 p-4">
                            <div class="flex items-center justify-between text-sm">
                                <span class="text-muted-foreground">Số lượng sản phẩm</span>
                                <span class="font-medium">{{ order.total_items }} SP</span>
                            </div>
                            <div v-if="parseFloat(order.shipping_cost) > 0"
                            ...
                            <div v-if="order.paid_at"
                                ...
                                <p class="text-[11px] font-medium tracking-wider text-green-600 uppercase">
                                    Đã thanh toán lúc
                                </p>
                    ...
                    <div class="space-y-4 rounded-xl border bg-muted/20 p-4">
                        <div v-if="order.notes" class="space-y-1">
                            <p class="text-[11px] font-bold tracking-tight text-muted-foreground uppercase">
                                Ghi chú đơn hàng
                            </p>
                        ...
                        <div v-if="order.accepted_by" class="flex items-center gap-2 border-t border-muted pt-2">
                            <div
                ...
                <div class="space-y-6 lg:col-span-8">
                            ...
                            <table class="w-full">
                                ...
                                <tbody class="divide-y">
                                    <tr v-for="item in order.items" :key="item.id"
                                        ...
                                        <td class="px-4 py-4 text-right text-muted-foreground tabular-nums">
                                            {{
                                                Number(
                                                    item.unit_price,
                                                ).toLocaleString('vi-VN')
                                ...
                                <tfoot v-if="order.items?.length" class="bg-muted/30">
                                    <tr>
                                        <td colspan="3"
                                            class="px-4 py-3 text-right text-sm font-medium text-muted-foreground">
                                            Tổng cộng
                                        </td>
                                        <td class="px-4 py-3 text-right text-lg font-bold text-primary tabular-nums">
                                            {{
                                                Number(
                                                    order.total_amount,
                                                ).toLocaleString('vi-VN')
                    ...
                    <!-- Invoices Section -->
                    <div v-if="order.invoices?.length" class="overflow-hidden rounded-xl border bg-card shadow-sm">
                        <div class="flex items-center gap-2 border-b bg-muted/30 px-4 py-3">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full">
                                ...
                                <tbody class="divide-y text-sm">
                                    <tr v-for="inv in order.invoices" :key="inv.id"
                                        class="transition-colors hover:bg-muted/20">
                                        <td class="px-4 py-3 font-mono text-[11px] font-bold">
                                            {{ inv.invoice_number }}
                                        </td>
                                        ...
                                        <td class="px-4 py-3 text-right font-medium text-green-600 tabular-nums">
                                            {{
                                                Number(
                                                    inv.amount_paid,
                                                ).toLocaleString('vi-VN')
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <Button v-if="canCreateShipments" variant="outline" size="sm"
                        ...
                        <div v-if="!order.shipments?.length"
                        ...
                        <div v-for="shipment in order.shipments" :key="shipment.id"
                            ...
                            <div class="flex items-center justify-between border-b bg-muted/20 px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <span class="font-mono text-sm font-bold text-primary">{{ shipment.shipment_number
                                        }}</span>
                                    <Badge :class="[
                                        'text-[10px]',
                                ...
                                <div class="flex items-center gap-2">
                                    <Button v-if="
                                        ...
                                        order.status !== 'cancelled'
                                    " variant="outline" size="sm" class="h-7 text-xs text-blue-600" @click="
                            ...
                            <div class="overflow-x-auto">
                                <table class="w-full text-sm">
                                    <thead>
                                        <tr class="bg-muted/10 text-xs tracking-wider text-muted-foreground uppercase">
    ...
    <ShipmentCreationDialog v-model="showShipmentsDialog" :order-id="order.id" @success="router.reload()" />
    <!-- Return Item Dialog -->
    <Dialog v-model:open="showReturnDialog">
        <DialogContent class="sm:max-w-md">
            ...
            <div class="space-y-4 py-4">
                ...
                <!-- Reason Input -->
                <div class="space-y-2">
                    <Label class="text-xs font-semibold tracking-wider text-muted-foreground uppercase">Lý do trả
                        hàng</Label>

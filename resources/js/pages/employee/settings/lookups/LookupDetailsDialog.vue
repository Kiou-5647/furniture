    ...
    <Dialog :open="open" @update:open="(val) => !val && emit('close')">
        <DialogContent class="max-w-[95vw] sm:max-w-[700px] max-h-[90vh] overflow-y-auto overflow-x-hidden p-4 sm:p-6">
            ...
            <div v-if="lookup" class="space-y-6 min-w-0">
                ...
                <div class="flex flex-col sm:flex-row items-start justify-between gap-4 pb-4 border-b min-w-0">
                    <div class="flex-1 space-y-2 w-full min-w-0">
                        <div class="flex items-start gap-2">
                            ...
                            <h3 class="text-lg font-semibold wrap-break-word">Tên hiển thị: {{ lookup.display_name }}</h3>
                        </div>
                        <div class="flex items-start gap-2">
                            <Hash class="h-3.5 w-3.5 text-muted-foreground shrink-0 mt-1" />
                            <code class="text-sm bg-muted px-2 py-0.5 rounded break-all whitespace-pre-wrap flex-1 min-w-0">Đường dẫn: {{ lookup.slug }}</code>
                        </div>
                ...
                <!-- Image Section -->
                <div v-if="lookup.image_path" class="space-y-2 min-w-0">
                ...
                <div class="space-y-3 min-w-0">
                    ...
                    <div class="grid gap-3 min-w-0">
                        <!-- SEO Title -->
                        <div v-if="lookup.metadata?.title" class="flex items-start gap-3 p-3 rounded-lg bg-muted/50 min-w-0">
                        ...
                        <div v-if="lookup.metadata?.canonical"
                            ...
                            <div class="flex-1 space-y-1 min-w-0">
                                ...
                                <code class="text-xs break-all text-blue-600 block whitespace-pre-wrap">{{ lookup.metadata.canonical }}</code>
                            </div>
                        </div>

                        <!-- Robots -->
                        <div v-if="lookup.metadata?.robots" class="flex items-start gap-3 p-3 rounded-lg bg-muted/50 min-w-0">
                            <Info class="h-4 w-4 text-muted-foreground shrink-0 mt-0.5" />
                            <div class="flex-1 space-y-1 min-w-0">
                                <p class="text-xs font-medium text-muted-foreground">Robots</p>
                                <Badge variant="outline" class="truncate max-w-full block text-center">{{ lookup.metadata.robots }}</Badge>
                            </div>
                        </div>
            ...
            <DialogFooter class="flex flex-col sm:flex-row gap-2 mt-6 justify-between">
                ...
                <div class="flex gap-2 w-full sm:w-auto order-2 sm:order-3">
                    <Button @click="emit('delete', lookup)" class="flex-1 sm:flex-none text-white bg-destructive hover:bg-destructive/90">

        ...
        <DialogContent
            ...
            <div class="absolute top-8 left-8 z-50 flex items-center gap-2">
                <Button variant="secondary" size="icon"
                    ...
                    @click="handleZoomOut" :disabled="zoomLevel <= 0.5">
                    <ZoomOut class="w-4 h-4" />
                </Button>
                <Button variant="secondary" size="icon"
                    class="w-9 h-9 bg-white/20 backdrop-blur-md border border-gray-500/30 hover:bg-white/30 text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    @click="handleZoomIn" :disabled="zoomLevel >= 3">
                    <ZoomIn class="w-4 h-4" />
                </Button>
                <Button variant="secondary" size="icon"
                    class="w-9 h-9 bg-white/20 backdrop-blur-md border border-gray-500/30 hover:bg-white/30 text-white shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    @click="handleReset" :disabled="zoomLevel === 1">
                    <Maximize2 class="w-4 h-4" />
                </Button>
                <span
                    class="text-white text-sm font-semibold bg-black/70 backdrop-blur-sm px-2.5 py-1 rounded-md border border-white/20 shadow-lg">
